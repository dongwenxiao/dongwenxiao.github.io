...
(function(undefined) {
    ...
    ************************************/
    ...
    function makeAs(input, model) {
        ...
        if (model._isUTC) {
            res = model.clone();
            diff =
                (moment.isMoment(input) || isDate(input)
                    ? +input
                    : +moment(input)) - +res;
    ...
    ************************************/
            ...
            case 'Do':
                if (input != null) {
                    datePartArray[DATE] = toInt(
                        parseInt(input.match(/\d{1,2}/)[0], 10)
                    );
                }
                break;
            // DAY OF YEAR
    ...
    function dateFromConfig(config) {
        ...
        config._d = (config._useUTC ? makeUTCDate : makeDate).apply(
            null,
            input
        );
    ...
    extend((moment.fn = Moment.prototype), {
        ...
        format: function(inputString) {
            var output = formatMoment(
                this,
                inputString || moment.defaultFormat
            );
            ...
            var now = time || moment(),
                ...
                format =
                    diff < -6
                        ...
                        : 'sameElse';
            return this.format(
                this.localeData().calendar(format, this, moment(now))
            );
        },
        ...
        dayOfYear: function(input) {
            var dayOfYear =
                round(
                    (moment(this).startOf('day') -
                        moment(this).startOf('year')) /
                        864e5
                ) + 1;
