(function() {
    ...
    (window.Math.randomInt = function(min, max) {
         ...
         *	x、y 可以是负数
         */
        // window.Math.getRadian = function(x,y,r){
        // 	var radian = Math.acos(y / r) * 180 / Math.PI,
        // 	flag = Number(x) >= 0 ? 1:-1;
        // 	radian *= flag;
        // 	return radian;
        // }
        (window.Math.getAngle = function(x, y, r) {
            var angle = (Math.acos(y / r) * 180) / Math.PI,
                flag = Number(x) >= 0 ? 1 : -1;
            angle *= flag;
            return angle;
        });
     ...
     * 角度转化弧度
     */
    window.Math.Angle2Radian = function(angle) {
        return (angle * 2 * Math.PI) / 360;
    };
    /**
     * 弧度转化角度
     */
    window.Math.Radian2Angle = function(radian) {
