...
cc.AsyncPool = function(srcObj, limit, iterator, onEnd, target) {
    ...
    self._handleItem = function() {
        var self = this;
        if (self._pool.length === 0 || self._workingSize >= self._limit) return;
...
cc.loader = {
    ...
    _loadJs4Dependency: function(baseDir, jsList, index, cb) {
        ...
        var self = this;
        self._createScript(
            cc.path.join(baseDir, jsList[index]),
            false,
            function(err) {
    ...
    _loadResIterator: function(item, index, cb) {
        ...
        if (!cc._urlRegExp.test(url)) {
            var basePath = loader.getBasePath
                ? loader.getBasePath()
                : self.resPath;
...
cc._initSys = function(config, CONFIG_KEY) {
    ...
    var isAndroid =
        ua.match(/android/i) || nav.platform.match(/android/i) ? true : false;
    ...
    var browserTypes =
        ua.match(
            /sogou|qzone|liebao|micromessenger|qqbrowser|ucbrowser|360 aphone|360browser|baiduboxapp|baidubrowser|maxthon|trident|oupeng|opera|miuibrowser|firefox/i
    ...
    (function(sys, config) {
        ...
        if (
            ...
            (userRenderMode === 0 &&
                supportWebGL &&
                shieldOs.indexOf(sys.os) === -1 &&
                shieldBrowser.indexOf(sys.browserType) === -1)
        )
            try {
        ...
        if (
            userRenderMode === 1 ||
            (userRenderMode === 0 && cc._supportRender === false)
        )
            try {
...
cc._setup = function(el, width, height) {
    ...
    if (cc._renderContext) {
        ...
        cc._renderContext = new cc.CanvasContextWrapper(
            localCanvas.getContext('2d')
        );
        cc._drawingUtil = cc.DrawingPrimitiveCanvas
            ? new cc.DrawingPrimitiveCanvas(cc._renderContext)
            : null;
    }
...
cc.game = {
    ...
    _stTime: function(callback) {
        ...
        var timeToCall = Math.max(
            0,
            cc.game._frameTime - (currTime - cc.game._lastTime)
        );
            ...
            if (cc._supportRender) {
                self._checkPrepare = setInterval(function() {
                    if (self._prepared) {
                        ...
                        self._eventHide =
                            self._eventHide ||
                            new cc.EventCustom(self.EVENT_HIDE);
    ...
    _initConfig: function() {
        ...
        var _init = function(cfg) {
            cfg[CONFIG_KEY.engineDir] =
                cfg[CONFIG_KEY.engineDir] || 'frameworks/cocos2d-html5';
        ...
        if (cc.Class) {
            ...
            loader.loadJson(ccModulesPath, function(err, modulesJson) {
                ...
                for (var i = 0, li = modules.length; i < li; i++) {
                    var arr = self._getJsListOfModule(
                        moduleMap,
                        modules[i],
                        engineDir
                    );
...
cc.loader.loadBinary = function(url, cb) {
    ...
    if (cc.loader.loadBinary._IEFilter) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                var fileContents = cc._convertResponseBodyToText(
                    xhr['responseBody']
                );
        ...
        xhr.onload = function() {
            xhr.readyState === 4 && xhr.status === 200
                ? cb(null, self._str2Uint8Array(xhr.responseText))
                : cb(errInfo);
        };
    }
    xhr.send(null);
        ...
        for (var idx = 0, li = arguments.length; idx < li; ++idx) {
            ...
            for (var name in prop) {
                ...
                if (releaseMode && isFunc && override && hasSuperCall) {
                    desc.value = ClassManager.compileSuper(
                        prop[name],
                        name,
                        classId
                    );
...
cc.rectContainsPoint = function(rect, point) {
    return (
        point.x >= cc.rectGetMinX(rect) &&
        point.x <= cc.rectGetMaxX(rect) &&
        point.y >= cc.rectGetMinY(rect) &&
        point.y <= cc.rectGetMaxY(rect)
    );
};
...
cc.rectUnion = function(rectA, rectB) {
    ...
    rect.width =
        Math.max(rectA.x + rectA.width, rectB.x + rectB.width) - rect.x;
    rect.height =
...
cc.rectIntersection = function(rectA, rectB) {
    ...
    intersection.width =
        Math.min(cc.rectGetMaxX(rectA), cc.rectGetMaxX(rectB)) -
        cc.rectGetMinX(intersection);
    intersection.height =
        Math.min(cc.rectGetMaxY(rectA), cc.rectGetMaxY(rectB)) -
        cc.rectGetMinY(intersection);
    return intersection;
};
...
cc.PlistParser = cc.SAXParser.extend({
    parse: function(xmlTxt) {
        ...
        if (plist.tagName !== 'plist') throw new Error('Not a plist file!');
        var node = null;
    ...
    _parseDict: function(node) {
        ...
        for (var i = 0, len = node.childNodes.length; i < len; i++) {
            ...
            else data[key] = this._parseNode(child);
        }
        return data;
    }
});
...
cc.$ = function(x) {
    ...
    if (el) {
        ...
        el.prependTo =
            ...
            function(x) {
                x.childNodes[0]
                    ? x.insertBefore(this, x.childNodes[0])
                    : x.appendChild(this);
                return this;
            };
        el.transforms =
...
cc._tmp.WebGLColor = function() {
    ...
    cc.Color = function(r, g, b, a, arrayBuffer, offset) {
        this._arrayBuffer =
            arrayBuffer || new ArrayBuffer(cc.Color.BYTES_PER_ELEMENT);
        this._offset = offset || 0;
        var locArrayBuffer = this._arrayBuffer,
            locOffset = this._offset,
            locElementLen = Uint8Array.BYTES_PER_ELEMENT;
        this._rU8 = new Uint8Array(locArrayBuffer, locOffset, 1);
        this._gU8 = new Uint8Array(
            locArrayBuffer,
            locOffset + locElementLen,
            1
        );
        this._bU8 = new Uint8Array(
            locArrayBuffer,
            locOffset + locElementLen * 2,
            1
        );
        this._aU8 = new Uint8Array(
            locArrayBuffer,
            locOffset + locElementLen * 3,
            1
        );
        this._rU8[0] = r || 0;
        this._gU8[0] = g || 0;
    ...
    cc.Vertex2F = function(x, y, arrayBuffer, offset) {
        this._arrayBuffer =
            arrayBuffer || new ArrayBuffer(cc.Vertex2F.BYTES_PER_ELEMENT);
        this._offset = offset || 0;
    ...
    cc.Vertex3F = function(x, y, z, arrayBuffer, offset) {
        this._arrayBuffer =
            arrayBuffer || new ArrayBuffer(cc.Vertex3F.BYTES_PER_ELEMENT);
        this._offset = offset || 0;
        var locArrayBuffer = this._arrayBuffer,
            locOffset = this._offset;
        this._xF32 = new Float32Array(locArrayBuffer, locOffset, 1);
        this._xF32[0] = x || 0;
        this._yF32 = new Float32Array(
            locArrayBuffer,
            locOffset + Float32Array.BYTES_PER_ELEMENT,
            1
        );
        this._yF32[0] = y || 0;
        this._zF32 = new Float32Array(
            locArrayBuffer,
            locOffset + Float32Array.BYTES_PER_ELEMENT * 2,
            1
        );
        this._zF32[0] = z || 0;
    };
    ...
    cc.Tex2F = function(u, v, arrayBuffer, offset) {
        this._arrayBuffer =
            arrayBuffer || new ArrayBuffer(cc.Tex2F.BYTES_PER_ELEMENT);
        this._offset = offset || 0;
    ...
    cc.Quad2 = function(tl, tr, bl, br, arrayBuffer, offset) {
        this._arrayBuffer =
            arrayBuffer || new ArrayBuffer(cc.Quad2.BYTES_PER_ELEMENT);
        this._offset = offset || 0;
        var locArrayBuffer = this._arrayBuffer,
            locElementLen = cc.Vertex2F.BYTES_PER_ELEMENT;
        this._tl = tl
            ? new cc.Vertex2F(tl.x, tl.y, locArrayBuffer, 0)
            : new cc.Vertex2F(0, 0, locArrayBuffer, 0);
        this._tr = tr
            ? new cc.Vertex2F(tr.x, tr.y, locArrayBuffer, locElementLen)
            : new cc.Vertex2F(0, 0, locArrayBuffer, locElementLen);
        this._bl = bl
            ? new cc.Vertex2F(bl.x, bl.y, locArrayBuffer, locElementLen * 2)
            : new cc.Vertex2F(0, 0, locArrayBuffer, locElementLen * 2);
        this._br = br
            ? new cc.Vertex2F(br.x, br.y, locArrayBuffer, locElementLen * 3)
            : new cc.Vertex2F(0, 0, locArrayBuffer, locElementLen * 3);
    };
    ...
    cc.V3F_C4B_T2F = function(
        ...
        this._arrayBuffer =
            arrayBuffer || new ArrayBuffer(cc.V3F_C4B_T2F.BYTES_PER_ELEMENT);
        this._offset = offset || 0;
        var locArrayBuffer = this._arrayBuffer,
            locOffset = this._offset,
        ...
        this._texCoords = texCoords
            ? new cc.Tex2F(
                  ...
                  texCoords.v,
                  locArrayBuffer,
                  locOffset + locElementLen + cc.Color.BYTES_PER_ELEMENT
              )
            : new cc.Tex2F(
                  0,
                  0,
    ...
    cc.V2F_C4B_T2F = function(
        ...
        this._arrayBuffer =
            arrayBuffer || new ArrayBuffer(cc.V2F_C4B_T2F.BYTES_PER_ELEMENT);
        this._offset = offset || 0;
        var locArrayBuffer = this._arrayBuffer,
            locOffset = this._offset,
            locElementLen = cc.Vertex2F.BYTES_PER_ELEMENT;
...
cc.color = function(r, g, b, a) {
    ...
    if (cc.isString(r)) return cc.hexToColor(r);
    if (cc.isObject(r))
...
cc.FontDefinition.prototype._getCanvasFontStr = function() {
    var lineHeight = !this.lineHeight.charAt
        ? this.lineHeight + 'px'
        : this.lineHeight;
    return (
        this.fontStyle +
...
if (cc._renderType === cc._RENDER_TYPE_WEBGL) {
    cc.assert(
        cc.isFunction(cc._tmp.WebGLColor),
...
cc.EGLView = cc.Class.extend({
    ...
    getScissorRect: function() {
        ...
        return cc.rect(
            (boxArr[0] - this._viewPortRect.x) / scaleX,
            (boxArr[1] - this._viewPortRect.y) / scaleY,
            boxArr[2] / scaleX,
            boxArr[3] / scaleY
        );
    },
    ...
    convertToLocationInView: function(tx, ty, relatedPos) {
        return {
            ...
            y:
                ...
                (relatedPos.top + relatedPos.height - ty)
        };
    },
    _convertMouseToLocationInView: function(point, relatedPos) {
        var locViewPortRect = this._viewPortRect,
            _t = this;
        point.x =
            (_t._devicePixelRatio * (point.x - relatedPos.left) -
                locViewPortRect.x) /
            _t._scaleX;
        point.y =
            (_t._devicePixelRatio *
                (relatedPos.top + relatedPos.height - point.y) -
                locViewPortRect.y) /
            _t._scaleY;
    },
    _convertTouchesWithScale: function(touches) {
        ...
        for (var i = 0; i < touches.length; i++) {
            ...
            selTouch._setPoint(
                (selPoint.x - locViewPortRect.x) / locScaleX,
                (selPoint.y - locViewPortRect.y) / locScaleY
            );
            selTouch._setPrevPoint(
                (selPrePoint.x - locViewPortRect.x) / locScaleX,
...
cc.ContainerStrategy = cc.Class.extend({
    ...
    _setupContainer: function(view, w, h) {
        ...
        var locCanvasElement = cc._canvas,
            locContainer = cc.container;
        ...
        if (view.isRetinaEnabled())
            devicePixelRatio = view._devicePixelRatio =
    ...
    var EqualToFrame = cc.ContainerStrategy.extend({
        apply: function(view) {
            this._setupContainer(
                view,
                view._frameSize.width,
                view._frameSize.height
            );
        }
    });
...
cc.screen = {
    ...
    init: function() {
        ...
        this._supportsFullScreen =
            typeof this._fn.requestFullscreen !== 'undefined';
        this._touchEvent =
    ...
    fullScreen: function() {
        if (!this._supportsFullScreen) return false;
            ...
            document[this._fn.fullscreenElement] === undefined ||
            document[this._fn.fullscreenElement] === null
        )
            return false;
        else return true;
    },
            ...
            for (var eventName in _pointerEventsMap) {
                (function(_pointerEvent, _touchEvent) {
                    cc._addEventListener(
                        ...
                        _pointerEvent,
                        function(event) {
                            var pos = selfPointer.getHTMLElementPosition(
                                element
                            );
...
cc.rectApplyAffineTransform = function(rect, anAffineTransform) {
    ...
    var bottomLeft = cc.pointApplyAffineTransform(
        left,
        bottom,
        anAffineTransform
    );
    var bottomRight = cc.pointApplyAffineTransform(
        right,
        bottom,
        anAffineTransform
    );
...
cc.pClamp = function(p, min_inclusive, max_inclusive) {
    return cc.p(
        cc.clampf(p.x, min_inclusive.x, max_inclusive.x),
        cc.clampf(p.y, min_inclusive.y, max_inclusive.y)
    );
};
...
cc.Touch = cc.Class.extend({
    ...
    _setPrevPoint: function(x, y) {
        if (y === undefined) this._prevPoint = cc.p(x.x, x.y);
        else this._prevPoint = cc.p(x || 0, y || 0);
    }
});
...
cc.EventListener.create = function(argObj) {
    ...
    if (listenerType === cc.EventListener.TOUCH_ONE_BY_ONE)
        ...
        listener = new cc._EventListenerCustom(
            argObj.eventName,
            argObj.callback
        );
...
cc._EventListenerFocus = cc.EventListener.extend({
    ...
    ctor: function() {
        var listener = function(event) {
            if (this.onFocusChanged)
                this.onFocusChanged(
                    event._widgetLoseFocus,
                    event._widgetGetFocus
                );
        };
        cc.EventListener.prototype.ctor.call(
            this,
...
cc.eventManager = {
    ...
    _sortEventListeners: function(listenerID) {
        ...
        if (dirtyFlag !== this.DIRTY_NONE) {
            ...
            if (dirtyFlag & this.DIRTY_SCENE_GRAPH_PRIORITY) {
                ...
                if (rootNode)
                    this._sortListenersOfSceneGraphPriority(
                        listenerID,
                        rootNode
                    );
    ...
    _updateListeners: function(event) {
        ...
        cc.assert(
            locInDispatch > 0,
            cc._LogInfos.EventManager__updateListeners
        );
    ...
    _visitTarget: function(node, isRootNode) {
        ...
        if (childrenCount > 0) {
            ...
            if (locNodeListenersMap[node.__instanceId] != null) {
                ...
                locGlobalZOrderNodeMap[node.getGlobalZOrder()].push(
                    node.__instanceId
                );
            }
            for (; i < childrenCount; i++) {
        ...
        if (isRootNode) {
            ...
            for (i = 0; i < zOrdersLen; i++) {
                ...
                for (j = 0; j < selZOrders.length; j++)
                    locNodePriorityMap[selZOrders[j]] = ++this
                        ._nodePriorityIndex;
            }
        ...
        if (listeners[type] !== undefined) {
            for (var i = 0, len = listeners.length; i < len; i++) {
                ...
                if (
                    ...
                    selListener.eventTarget === target
                )
                    return true;
            }
        }
...
cc._tmp.PrototypeCCNode = function() {
    ...
    cc.defineGetterSetter(
        _p,
        'ignoreAnchor',
        _p.isIgnoreAnchorPointForPosition,
        _p.ignoreAnchorPointForPosition
    );
    _p.tag;
    _p.userData;
    _p.userObject;
    ...
    cc.defineGetterSetter(
        _p,
        'cascadeOpacity',
        _p.isCascadeOpacityEnabled,
        _p.setCascadeOpacityEnabled
    );
    _p.color;
    ...
    _updateDisplayOpacity: function(parentOpacity) {
        ...
        if (this._cascadeOpacityEnabledDirty && !node._cascadeOpacityEnabled) {
            ...
            this._displayedOpacity =
                (node._realOpacity * parentOpacity) / 255.0;
            if (node._cascadeOpacityEnabled) {
                selChildren = node._children;
                for (i = 0, len = selChildren.length; i < len; i++) {
                    item = selChildren[i];
                    if (item && item._renderCmd) {
                        item._renderCmd._updateDisplayOpacity(
                            this._displayedOpacity
                        );
                        item._renderCmd._updateColor();
                    }
...
(function() {
    ...
    var proto = (cc.Node.CanvasRenderCmd.prototype = Object.create(
        cc.Node.RenderCmd.prototype
    ));
    ...
    proto.transform = function(parentCmd, recursive) {
        ...
        if (recursive) {
            ...
            if (!locChildren || locChildren.length === 0) return;
            var i, len;
            for (i = 0, len = locChildren.length; i < len; i++) {
    ...
    proto.getNodeToParentTransform = function() {
        ...
        if (
            ...
            t.c = c;
            t.d = d;
            var lScaleX = node._scaleX,
                lScaleY = node._scaleY;
            var appX = this._anchorPointInPoints.x,
                appY = this._anchorPointInPoints.y;
            var sx =
                    lScaleX < 0.000001 && lScaleX > -0.000001
                        ? 0.000001
                        : lScaleX,
                sy =
                    lScaleY < 0.000001 && lScaleY > -0.000001
                        ? 0.000001
                        : lScaleY;
    ...
    proto.visit = function(parentCmd) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; i++) {
                child = children[i];
                if (child._localZOrder < 0) child._renderCmd.visit(this);
                else break;
            }
            cc.renderer.pushRenderCommand(this);
            for (; i < len; i++) children[i]._renderCmd.visit(this);
        } else {
            cc.renderer.pushRenderCommand(this);
        }
        this._dirtyFlag = 0;
    ...
    proto._syncStatus = function(parentCmd) {
        ...
        if (colorDirty) this._syncDisplayColor();
        if (opacityDirty) this._syncDisplayOpacity();
    ...
    cc.Node.CanvasRenderCmd._getCompositeOperationByBlendFunc = function(
        blendFunc
    ) {
        if (!blendFunc) return 'source-over';
        else {
            if (
                (blendFunc.src === cc.SRC_ALPHA && blendFunc.dst === cc.ONE) ||
                (blendFunc.src === cc.ONE && blendFunc.dst === cc.ONE)
            )
                return 'lighter';
            else if (
                blendFunc.src === cc.ZERO &&
                blendFunc.dst === cc.SRC_ALPHA
            )
...
cc._tmp.WebGLTextureCache = function() {
    ...
    _p.addImage = function(url, cb, target) {
        ...
        tex = locTexs[url] = new cc.Texture2D();
        tex.url = url;
        var loadFunc = cc.loader._checkIsImageURL(url)
            ? cc.loader.load
            : cc.loader.loadImg;
...
if (cc._renderType === cc._RENDER_TYPE_CANVAS) {
    cc.Texture2D = cc.Class.extend({
        ...
        setTexParameters: function(texParams, magFilter, wrapS, wrapT) {
            ...
            if (
                texParams.wrapS === cc.REPEAT &&
                texParams.wrapT === cc.REPEAT
            ) {
...
cc.textureCache = {
    ...
    getTextureForKey: function(textureKeyName) {
        return (
            ...
            this._textures[cc.loader._aliases[textureKeyName]]
        );
    },
    ...
    getTextureColors: function(texture) {
        ...
        if (!key) {
            if (texture instanceof HTMLImageElement) key = texture.src;
            else key = this._generalTextureKey();
        }
...
cc.LoaderScene = cc.Scene.extend({
    ...
    init: function() {
        ...
        var logoWidth = 160;
        var logoHeight = 200;
        var bgLayer = (self._bgLayer = new cc.LayerColor(
            cc.color(32, 32, 32, 255)
        ));
...
cc.Layer = cc.Node.extend({
    ...
    _createRenderCmd: function() {
        ...
        else return new cc.Layer.WebGLRenderCmd(this);
    }
});
...
cc.LayerColor = cc.Layer.extend({
    ...
    _createRenderCmd: function() {
        ...
        else return new cc.LayerColor.WebGLRenderCmd(this);
    }
});
...
cc.LayerGradient = cc.LayerColor.extend({
    ...
    init: function(start, end, v, stops) {
        ...
        cc.LayerColor.prototype.init.call(
            _t,
            cc.color(start.r, start.g, start.b, 255)
        );
        this._renderCmd.setDirtyFlag(
    ...
    _createRenderCmd: function() {
        ...
        else return new cc.LayerGradient.WebGLRenderCmd(this);
    }
});
...
(function() {
    ...
    cc.defineGetterSetter(
        proto,
        'startOpacity',
        proto.getStartOpacity,
        proto.setStartOpacity
    );
    proto.endOpacity;
    cc.defineGetterSetter(
        proto,
...
(function() {
    ...
    var proto = (cc.Layer.CanvasRenderCmd.prototype = Object.create(
    ...
    proto.transform = function(parentCmd, recursive) {
        ...
        var a = wt.a,
            ...
            tx = wt.tx,
            ty = wt.ty;
        cc.Node.CanvasRenderCmd.prototype.transform.call(
            this,
            parentCmd,
            recursive
        );
...
(function() {
    ...
    var proto = (cc.LayerColor.CanvasRenderCmd.prototype = Object.create(
    ...
    proto._getBoundingBoxForBake = function() {
        ...
        var trans = node.getNodeToWorldTransform();
        rect = cc.rectApplyAffineTransform(
            rect,
...
(function() {
    ...
    var proto = (cc.LayerGradient.CanvasRenderCmd.prototype = Object.create(
    ...
    proto.rendering = function(ctx, scaleX, scaleY) {
        ...
        var gradient = context.createLinearGradient(
            this._startPoint.x * scaleX,
            this._startPoint.y * scaleY,
            this._endPoint.x * scaleX,
            this._endPoint.y * scaleY
        );
        if (node._colorStops) {
...
cc.Sprite = cc.Node.extend({
            ...
            return this.initWithTexture(
                ...
                rect ||
                    cc.rect(
                        0,
                        0,
                        tex._contentSize.width,
                        tex._contentSize.height
                    )
            );
        } else {
            if (!rect) {
    ...
    initWithTexture: function(texture, rect, rotated, counterclockwise) {
        ...
        if (!locTextureLoaded) {
            ...
            if (_t.texture) _t.texture.removeEventListener('load', _t);
            texture.addEventListener(
                'load',
                _t._renderCmd._textureLoadedCallback,
                _t
            );
            _t.texture = texture;
            return true;
    ...
    setTexture: function(texture) {
        ...
        if (cc.isString(texture)) {
            ...
            cc.assert(
                ...
                cc._LogInfos.Sprite_setTexture_2
            );
            this._changeRectWithTexture(texture, oldTexture);
            this._renderCmd._setTexture(texture);
        }
    ...
    _changeRectWithTexture: function(texture, oldTexture) {
        var textureRect = cc.rect(
                0,
                0,
                texture._contentSize.width,
                texture._contentSize.height
            ),
            oldTextureContentSize = oldTexture
                ? oldTexture._contentSize
                : cc.size(),
        ...
        if (
            ...
            if (texture.url) {
                if (
                    nodeWidth !== oldTextureWidth &&
                    nodeHeight !== oldTextureHeight &&
...
(function() {
    ...
    var proto = (cc.Sprite.CanvasRenderCmd.prototype = Object.create(
        cc.Node.CanvasRenderCmd.prototype
    ));
    ...
    proto._handleTextureForRotatedTexture = function(
        texture,
        rect,
        rotated,
        counterclockwise
    ...
    proto.rendering = function(ctx, scaleX, scaleY) {
        ...
        if (node._texture) {
            ...
            if (node._texture._pattern !== '') {
                wrapper.setFillStyle(
                    context.createPattern(image, node._texture._pattern)
                );
                context.fillRect(
                    locX * scaleX,
                    locY * scaleY,
    ...
    if (!cc.sys._supportCanvasNewBlendModes) {
        proto._updateColor = function() {
            var node = this._node,
                displayedColor = this._displayedColor;
            if (
                displayedColor.r === 255 &&
                displayedColor.g === 255 &&
                displayedColor.b === 255
            ) {
            ...
            if (locTexture && locRect.validRect && this._originalTexture) {
                ...
                if (cacheTextureForColor) {
                    ...
                    if (
                        locElement instanceof HTMLCanvasElement &&
                        !this._rectRotated &&
                        !this._newTextureWhenChangeColor
                    )
                        ...
                        locElement = cc.Sprite.CanvasRenderCmd._generateTintImage(
                            locElement,
                            cacheTextureForColor,
                            displayedColor,
                            locRect
        ...
        proto._updateColor = function() {
            ...
            if (locTexture && locRect.validRect && this._originalTexture) {
                ...
                if (
                    ...
                    locElement = cc.Sprite.CanvasRenderCmd._generateTintImageWithMultiply(
                        this._originalTexture._htmlElementObj,
                        displayedColor,
                        locRect
                    );
    ...
    proto.updateTransform = function() {
        ...
        if (node.dirty) {
            ...
            if (
                !node._visible ||
                (locParent &&
                    locParent !== node._batchNode &&
                    locParent._shouldBeHidden)
            ) {
    ...
    cc.Sprite.CanvasRenderCmd._generateTintImageWithMultiply = function(
        image,
        color,
        rect,
        renderCanvas
    ...
    cc.Sprite.CanvasRenderCmd._generateTintImage = function(
        texture,
        tintedImgCache,
        color,
        rect,
    ...
    cc.Sprite.CanvasRenderCmd._generateTextureCacheForColor = function(
        texture
    ) {
    ...
    cc.Sprite.CanvasRenderCmd._generateTextureCacheForColor.canvas = cc.newElement(
        'canvas'
    );
    cc.Sprite.CanvasRenderCmd._generateTextureCacheForColor.tempCanvas = cc.newElement(
        'canvas'
    );
    cc.Sprite.CanvasRenderCmd._generateTextureCacheForColor.tempCtx = cc.Sprite.CanvasRenderCmd._generateTextureCacheForColor.tempCanvas.getContext(
        '2d'
    );
...
cc.BakeSprite = cc.Sprite.extend({
    ...
    resetCanvasSize: function(sizeOrWidth, height) {
        ...
        this.setTextureRect(
            cc.rect(0, 0, sizeOrWidth, height),
            false,
            null,
            false
        );
...
cc.animationCache = {
    ...
    _addAnimationsWithDictionary: function(dictionary, plist) {
        ...
        switch (version) {
            ...
            default:
                cc.log(
                    cc._LogInfos.animationCache__addAnimationsWithDictionary_2
                );
                break;
        }
    },
                ...
                texture.addEventListener(
                    ...
                    function(sender) {
                        ...
                        if (
                            ...
                            this.setRect(
                                cc.rect(0, 0, rect.width, rect.height)
                            );
                        }
                        var locRect = this._rect;
...
cc.SpriteFrame.create = function(
    filename,
    rect,
    rotated,
    offset,
...
cc.SpriteFrame._frameWithTextureForCanvas = function(
    ...
    cc._pointPixelsToPointsOut(
        spriteFrame._offsetInPixels,
        spriteFrame._offset
    );
...
cc.spriteFrameCache = {
    ...
    _parseFrameConfig: function(dict) {
        ...
        for (var key in tempFrames) {
            ...
            if (format == 0) {
                ...
                var textureRect = this._rectFromString(
                    frameDict['textureRect']
                );
                if (spriteSize) {
                    textureRect = cc.rect(
                        textureRect.x,
                ...
                tempFrame.offset = this._pointFromString(
                    frameDict['spriteOffset']
                );
                tempFrame.size = this._sizeFromString(
            ...
            if (!spriteFrame) {
                ...
                if (aliases) {
                    ...
                    for (var i = 0, li = aliases.length; i < li; i++) {
                        ...
                        if (spAliases[alias])
                            cc.log(
                                cc._LogInfos.spriteFrameCache_addSpriteFrames,
                                alias
                            );
                        spAliases[alias] = key;
                    }
    ...
    addSpriteFrames: function(url, texture) {
        ...
        var frameConfig =
            this._frameConfigCache[url] || this._getFrameConfig(url);
        this._createSpriteFrames(url, frameConfig, texture);
    },
...
cc.Director = cc.Class.extend({
    ...
    drawScene: function() {
        ...
        if (this._runningScene) {
            if (renderer.childrenOrderDirty === true) {
                ...
                renderer.resetFlag();
            } else if (renderer.transformDirty() === true) renderer.transform();
    ...
    setNotificationNode: function(node) {
        ...
        if (!node) cc.renderer.childrenOrderDirty = true;
    },
...
if (cc._renderType === cc._RENDER_TYPE_CANVAS) {
    ...
    _p._createStatsLabel = function() {
        ...
        else fontSize = 0 | ((_t._winSizeInPoints.width / 320) * 24);
        ...
        _t._drawsLabel.setPosition(
            _t._drawsLabel.width / 2 + locStatsPosition.x,
            (_t._drawsLabel.height * 5) / 2 + locStatsPosition.y
        );
        _t._SPFLabel.setPosition(
            _t._SPFLabel.width / 2 + locStatsPosition.x,
            (_t._SPFLabel.height * 3) / 2 + locStatsPosition.y
        );
        _t._FPSLabel.setPosition(
            _t._FPSLabel.width / 2 + locStatsPosition.x,
...
cc.HashTimerEntry = cc.hashSelectorEntry = function(
    timers,
    target,
    timerIndex,
    currentTimer,
...
cc.TimerTargetCallback = cc.Timer.extend({
    ...
    trigger: function() {
        if (this._callback) this._callback.call(this._target, this._elapsed);
    },
    cancel: function() {
...
cc.Scheduler = cc.Class.extend({
    ...
    schedule: function(callback, target, interval, repeat, delay, paused, key) {
        ...
        if (isSelector === false) cc.assert(key, 'key should not be empty!');
        var element = this._hashForTimers[target.__instanceId];
        if (!element) {
            element = new cc.HashTimerEntry(
                null,
                target,
                0,
                null,
    ...
    unscheduleAllForTarget: function(target) {
        ...
        if (element) {
            if (
                element.timers.indexOf(element.currentTimer) > -1 &&
                !element.currentTimerSalvaged
            ) {
                element.currentTimerSalvaged = true;
            }
...
cc._tmp.PrototypeLabelTTF = function() {
    ...
    cc.defineGetterSetter(
        _p,
        'textAlign',
        _p.getHorizontalAlignment,
        _p.setHorizontalAlignment
    );
    _p.verticalAlign;
    cc.defineGetterSetter(
        _p,
        'verticalAlign',
        _p.getVerticalAlignment,
        _p.setVerticalAlignment
    );
    _p.fontSize;
    ...
    cc.defineGetterSetter(
        _p,
        'shadowOffsetX',
        _p._getShadowOffsetX,
        _p._setShadowOffsetX
    );
    _p.shadowOffsetY;
    cc.defineGetterSetter(
        _p,
        'shadowOffsetY',
        _p._getShadowOffsetY,
        _p._setShadowOffsetY
    );
    _p.shadowOpacity;
    cc.defineGetterSetter(
        _p,
        'shadowOpacity',
        _p._getShadowOpacity,
        _p._setShadowOpacity
    );
    _p.shadowBlur;
    cc.defineGetterSetter(
        _p,
...
cc.LabelTTF = cc.Sprite.extend({
    ...
    _enableShadowNoneColor: function(
        shadowOffsetX,
        shadowOffsetY,
        shadowOpacity,
        shadowBlur
    ...
    _updateWithTextDefinition: function(textDefinition, mustUpdateTexture) {
        ...
        if (textDefinition.strokeEnabled)
            this.enableStroke(
                textDefinition.strokeStyle,
                textDefinition.lineWidth
            );
    ...
    setTextureRect: function(rect, rotated, untrimmedSize) {
        cc.Sprite.prototype.setTextureRect.call(
            this,
            rect,
            rotated,
            untrimmedSize,
    ...
    _createRenderCmd: function() {
        ...
        else return new cc.LabelTTF.WebGLRenderCmd(this);
    },
...
else cc.LabelTTF._SHADER_PROGRAM = cc.SHADER_POSITION_TEXTUREA8COLOR;
...
cc.LabelTTF.__getFontHeightByDiv = function(fontName, fontSize) {
    if (fontName instanceof cc.FontDefinition) {
        ...
        var clientHeight =
            cc.LabelTTF.__fontHeightCache[fontDef._getCanvasFontStr()];
...
(function() {
    ...
    proto._setFontStyle = function(
        ...
        if (fontNameOrFontDef instanceof cc.FontDefinition) {
            ...
            this._fontClientHeight = cc.LabelTTF.__getFontHeightByDiv(
                fontNameOrFontDef
            );
        } else {
            this._fontStyleStr =
    ...
    proto._updateTexture = function() {
        this._dirtyFlag =
            (this._dirtyFlag & cc.Node._dirtyFlags.textDirty) ^ this._dirtyFlag;
        var node = this._node;
    ...
    proto._updateTTF = function() {
        ...
        if (node._strokeEnabled)
            locStrokeShadowOffsetX = locStrokeShadowOffsetY =
                node._strokeSize * 2;
        ...
        if (locDimensionsWidth === 0) {
            if (this._isMultiLine)
                locSize = cc.size(
                    Math.ceil(
                        Math.max.apply(Math, locLineWidth) +
                            locStrokeShadowOffsetX
                    ),
                    Math.ceil(
                        this._fontClientHeight * this._strings.length +
                ...
                locSize = cc.size(
                    Math.ceil(
                        this._measure(node._string) + locStrokeShadowOffsetX
                    ),
                    Math.ceil(this._fontClientHeight + locStrokeShadowOffsetY)
                );
        } else {
            if (node._dimensions.height === 0) {
                if (this._isMultiLine)
                    locSize = cc.size(
                        ...
                        Math.ceil(
                            node.getLineHeight() * this._strings.length +
                                locStrokeShadowOffsetY
                        )
                    );
                else
                    locSize = cc.size(
                        Math.ceil(locDimensionsWidth + locStrokeShadowOffsetX),
                        Math.ceil(node.getLineHeight() + locStrokeShadowOffsetY)
                    );
            } else {
                locSize = cc.size(
                    Math.ceil(locDimensionsWidth + locStrokeShadowOffsetX),
                    Math.ceil(node._dimensions.height + locStrokeShadowOffsetY)
                );
            }
        }
...
(function() {
    ...
    proto.updateStatus = function() {
        ...
        if (colorDirty) {
            this._updateColor();
        } else if (locFlag & flags.textDirty) this._updateTexture();
    ...
    proto._syncStatus = function(parentCmd) {
        ...
        if (locFlag & flags.transformDirty) this.transform(parentCmd);
    };
...
cc.base = function(me, opt_methodName, var_args) {
    ...
    if (caller.superClass_) {
        ret = caller.superClass_.constructor.apply(
            me,
...
cc.rendererCanvas = {
    ...
    _renderingToCacheCanvas: function(ctx, instanceID, scaleX, scaleY) {
        ...
        var locCmds = this._cacheToCanvasCmds[instanceID],
            i,
            len;
...
if (cc._renderType === cc._RENDER_TYPE_CANVAS) cc.renderer = cc.rendererCanvas;
(function() {
...
cc.ActionManager = cc.Class.extend({
    ...
    removeAllActionsFromTarget: function(target, forceDelete) {
        ...
        if (element) {
            if (
                element.actions.indexOf(element.currentAction) !== -1 &&
        ...
        for (var elt = 0; elt < locTargets.length; elt++) {
            ...
            if (!locCurrTarget.paused) {
                for (
                    ...
                    (locCurrTarget.actions ? locCurrTarget.actions.length : 0);
                    locCurrTarget.actionIndex++
                ) {
            ...
            if (
                this._currentTargetSalvaged &&
                locCurrTarget.actions.length === 0
            ) {
...
cc.MoveTo = cc.MoveBy.extend({
    ...
    initWithDuration: function(duration, position, y) {
        if (
            cc.MoveBy.prototype.initWithDuration.call(
                this,
                duration,
                position,
                y
...
cc.SkewBy = cc.SkewTo.extend({
    ...
    initWithDuration: function(t, deltaSkewX, deltaSkewY) {
        var ret = false;
        if (
            cc.SkewTo.prototype.initWithDuration.call(
                this,
                t,
                deltaSkewX,
                deltaSkewY
...
cc.FadeTo = cc.ActionInterval.extend({
    ...
    update: function(time) {
        ...
        this.target.opacity =
            fromOpacity + (this._toOpacity - fromOpacity) * time;
    },
    startWithTarget: function(target) {
        cc.ActionInterval.prototype.startWithTarget.call(this, target);
...
cc.Animate = cc.ActionInterval.extend({
    ...
    initWithAnimation: function(animation) {
        ...
        if (this.initWithDuration(singleDuration * animation.getLoops())) {
            ...
            var newUnitOfTimeValue =
                singleDuration / animation.getTotalDelayUnits();
        ...
        for (var i = this._nextFrame; i < numberOfFrames; i++) {
            if (locSplitTimes[i] <= dt) {
                ...
                this.target.setSpriteFrame(
                    frames[_currFrameIndex].getSpriteFrame()
                );
                this._nextFrame = i + 1;
            } else {
                break;
...
cc.CallFunc = cc.ActionInstant.extend({
    ...
    ctor: function(selector, selectorTarget, data) {
        ...
        if (selector !== undefined) {
            if (selectorTarget === undefined) this.initWithFunction(selector);
    ...
    initWithFunction: function(selector, selectorTarget, data) {
        ...
        } else if (selector) this._function = selector;
        return true;
    },
    execute: function() {
        if (this._callFunc != null)
            this._callFunc.call(this._selectorTarget, this.target, this._data);
        else if (this._function) this._function.call(null, this.target);
    },
    update: function(dt) {
        this.execute();
    },
...
cc.OrbitCamera = cc.ActionCamera.extend({
    ...
    sphericalRadius: function() {
        ...
        var x = eyeXYZ.x - centerXYZ.x,
            y = eyeXYZ.y - centerXYZ.y,
            z = eyeXYZ.z - centerXYZ.z;
        var r = Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2) + Math.pow(z, 2));
        var s = Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2));
        if (s === 0.0) s = cc.FLT_EPSILON;
        if (r === 0.0) r = cc.FLT_EPSILON;
        zenith = Math.acos(z / r);
        if (x < 0) azimuth = Math.PI - Math.asin(y / s);
        else azimuth = Math.asin(y / s);
...
cc._easeElasticInObj = {
    easing: function(dt) {
        ...
        return (
            -Math.pow(2, 10 * dt) *
            Math.sin(((dt - 0.3 / 4) * Math.PI * 2) / 0.3)
        );
    },
    reverse: function() {
        ...
        return {
            ...
            easing: function(dt) {
                ...
                return (
                    -Math.pow(2, 10 * dt) *
                    Math.sin(
                        ((dt - this._period / 4) * Math.PI * 2) / this._period
                    )
                );
            },
            reverse: function() {
...
cc.EaseBackInOut = cc.ActionEase.extend({
    update: function(dt) {
        ...
        if (dt < 1) {
            this._inner.update(
                (dt * dt * ((overshoot + 1) * dt - overshoot)) / 2
            );
        } else {
            dt = dt - 2;
            this._inner.update(
...
cc._easeBackInOutObj = {
    easing: function(time1) {
        ...
        if (time1 < 1) {
            ...
            time1 = time1 - 2;
            return (
                (time1 * time1 * ((overshoot + 1) * time1 + overshoot)) / 2 + 1
            );
        }
    },
    reverse: function() {
...
cc.easeBezierAction = function(p0, p1, p2, p3) {
    return {
        easing: function(time) {
            return cc.EaseBezierAction.prototype._updateTime(
                p0,
                p1,
                p2,
                p3,
    ...
    if (cc.sys.isMobile) {
        if (cc.sys.os !== cc.sys.OS_IOS)
            window.__audioSupport =
                supportTable[sys.browserType] || supportTable['common'];
        else window.__audioSupport = supportTable[sys.BROWSER_TYPE_SAFARI];
    } else {
...
cc.Audio = cc.Class.extend({
    ...
    getPlaying: function() {
        ...
        if (!sourceNode || !sourceNode['playbackState']) return true;
        return (
    ...
    _playOfWebAudio: function(offset) {
        ...
        if (this._manualLoop) audio.loop = false;
        else audio.loop = this.loop;
    ...
    pause: function() {
        if (this.getPlaying() === false) return;
        this._playing = false;
        this._pause = true;
        if (this._AUDIO_TYPE === 'AUDIO') {
            this._pauseOfAudio();
...
(function(polyfill) {
    ...
    var loader = {
        ...
        load: function(realUrl, url, res, cb) {
            ...
            var audio;
            if (loader.cache[url]) return cb(null, loader.cache[url]);
            if (SWA) {
                try {
                    ...
                    if (polyfill.NEED_MANUAL_LOOP) audio._manualLoop = true;
                } catch (err) {
                    SWA = false;
                    cc.log("browser don't support web audio");
    ...
    cc.audioEngine = {
        ...
        playEffect: function(url, loop) {
            ...
            if (effectList[i]) {
                ...
                cc.log(
                    'Error: %s greater than %d',
                    url,
                    this._maxAudioInstance
                );
            } else {
                var audio = loader.cache[url];
                if (!audio) {
    ...
    if (!SWC) {
        var reBGM = function() {
            ...
            if (bg && bg._touch === false && bg._playing && bg.getPlaying()) {
                bg._touch = true;
                bg.play(0, bg.loop);
                !polyfill.REPLAY_AFTER_TOUCH &&
                    cc._canvas.removeEventListener('touchstart', reBGM);
            }
        };
        setTimeout(function() {
            if (cc._canvas) {
...
cc.MenuItemAtlasFont.create = function(
    value,
    charMapFile,
    itemWidth,
    itemHeight,
    ...
    return new cc.MenuItemAtlasFont(
        value,
        charMapFile,
        itemWidth,
        itemHeight,
...
cc.MenuItemSprite = cc.MenuItem.extend({
    ...
    initWithNormalSprite: function(
        ...
        if (locNormalImage) {
            ...
            if (
                locNormalImage.textureLoaded &&
                !locNormalImage.textureLoaded()
            ) {
                locNormalImage.addEventListener(
                    'load',
                    function(sender) {
                        this.width = sender.width;
                        this.height = sender.height;
...
cc.MenuItemImage = cc.MenuItemSprite.extend({
    ctor: function(normalImage, selectedImage, three, four, five) {
        ...
        if (normalImage === undefined || normalImage === null) {
            ...
            cc.MenuItemSprite.prototype.ctor.call(
                this,
                normalSprite,
                selectedSprite,
                disabledSprite,
...
cc.MenuItemImage.create = function(
    normalImage,
    selectedImage,
    three,
    four,
...
cc.Menu = cc.Layer.extend({
    ...
    alignItemsVerticallyWithPadding: function(padding) {
        ...
        if (locChildren && locChildren.length > 0) {
            for (i = 0, len = locChildren.length; i < len; i++)
                height +=
                    locChildren[i].height * locChildren[i].scaleY + padding;
            var y = height / 2.0;
            for (i = 0, len = locChildren.length; i < len; i++) {
    ...
    alignItemsInRows: function() {
        ...
        if (locChildren && locChildren.length > 0) {
            for (i = 0, len = locChildren.length; i < len; i++) {
                ...
                tmp = child.width;
                columnWidth =
                    columnWidth >= tmp || isNaN(tmp) ? columnWidth : tmp;
...
cc.Menu.create = function(menuItems) {
    ...
    if (argc > 0 && arguments[argc - 1] == null)
        cc.log('parameters should not be ending with null in Javascript');
    var ret;
    if (argc === 0) ret = new cc.Menu();
    else if (argc === 1) ret = new cc.Menu(menuItems);
    else ret = new cc.Menu(Array.prototype.slice.call(arguments, 0));
    return ret;
};
