...
var ResultLayer = cc.Layer.extend({
    ...
    initResultData: function(
        ...
        var bRet = true;
        if (gameScore > SCORE_MAX) gameScore = SCORE_MAX;
        else if (gameScore < 0) gameScore = 0;
        ...
        for (var starIndex = 0; starIndex < 3; starIndex++) {
            if (
                this.mStarSprites[starIndex] == null &&
                this.mGameScore > this.mStarScores[starIndex]
            ) {
                this.showStar(starIndex);
            }
        }
        if (bRet == false) cc.log('GameResult Error');
    },
    showStar: function(starIndex) {
        ...
        this.mStarSprites[starIndex].runAction(
        ...
        if (this.mIgnoreTouch == false) {
            ...
            switch (sender.getTag()) {
                case eGameResultMenuTag.Restart:
                    ...
                    var matrixLayer = cc.Director.getInstance()
                        .getRunningScene()
                        .getChildByTag(111);
                    matrixLayer.clearMsgListener();
                    cc.Director.getInstance().replaceScene(
                        cc.TransitionSlideInT.create(0.4, nextScene)
                    );
                    break;
            }
        }
