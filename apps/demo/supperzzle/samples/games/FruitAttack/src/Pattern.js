...
var PatternSprite = cc.Sprite.extend({
    ...
    init: function(type, extraAttr) {
        ...
        if (this._super()) {
            ...
            if (extraAttr == ePatternExtraAttr.Stone) this.m_ePatternType = -1;
            else this.m_ePatternType = type;
            if (
                this.m_ePatternType == -1 ||
                extraAttr == ePatternExtraAttr.Freeze
            )
                this.m_bSwapEnable = false;
            else this.m_bSwapEnable = true;

            switch (this.m_eExtraAttr) {
                case ePatternExtraAttr.Bomb: {
                    this.m_extraTypeSpr = cc.Sprite.createWithSpriteFrameName(
                        'pattern_mark_explode.png'
                    );
                    break;
                }
                case ePatternExtraAttr.Freeze: {
                    this.m_extraTypeSpr = cc.Sprite.createWithSpriteFrameName(
                        'pattern_mark_freeze.png'
                    );
                    break;
                }
                default:
            ...
            if (this.m_extraTypeSpr != null) {
                ...
                this.m_extraTypeSpr.setPosition(
                    size.width / 2,
                    size.height / 2
                );
    ...
    destroyPattern: function(frams) {
        ...
        var effectSprite = cc.Sprite.createWithSpriteFrameName(
            'pattern_destroy_00.png'
        );
    ...
    swapTo: function(duration, position) {
        if (this.g_ePatternStatus === ePatternStatus.Normal) {
            this.g_ePatternStatus = ePatternStatus.Move;
            this.runAction(
                cc.MoveTo.create(duration, cc.p(position.x, position.y))
            );
        }
    },
    onMoveEnd: function() {
    ...
    containsTouchLocation: function(touch) {
        ...
        if (lx > -22.5 && lx < 22.5 && ly > -22.5 && ly < 22.5) return true;
        return false;
    },
    onTouchBegan: function(touch, event) {
        if (
            ...
            if (
                this.m_eExtraAttr == ePatternExtraAttr.Normal ||
                this.m_eExtraAttr == ePatternExtraAttr.Bomb
            ) {
    ...
    onTouchMoved: function(touch, event) {
        if (
            ...
            if (lx > 45) {
                ...
                if (ly > lx) this.m_eSwapDirection = eSwapDirection.Up;
                else if (ly + lx < 0)
                    this.m_eSwapDirection = eSwapDirection.Down;
                else this.m_eSwapDirection = eSwapDirection.Right;
                gNotification.postNotification(MSG_SWAP_PATTERN, this);
            } else if (lx < -45) {
                this.m_bHandleTouch = false;
                if (ly < lx) this.m_eSwapDirection = eSwapDirection.Down;
                else if (ly + lx > 0) this.m_eSwapDirection = eSwapDirection.Up;
                else this.m_eSwapDirection = eSwapDirection.Left;
                gNotification.postNotification(MSG_SWAP_PATTERN, this);
            } else if (ly > 45) {
                this.m_bHandleTouch = false;
    ...
    onEnter: function() {
        ...
        else cc.registerTargettedDelegate(1, true, this);
        this._super();
    },
    onExit: function() {
