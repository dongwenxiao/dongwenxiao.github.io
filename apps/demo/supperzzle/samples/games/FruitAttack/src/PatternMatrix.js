...
var PatternMatrix = cc.Layer.extend({
    ...
    init: function() {
        ...
        if (this._super()) {
            ...
            gNotification.addObserver(
                this,
                this.onSwapTwoPattern,
                MSG_SWAP_PATTERN
            );

            this.mPatternsPos = this.createIntArray(
                MATRIX_ROW_MAX,
                MATRIX_COL_MAX,
                null
            );
            this.mPatternsSpr = this.createIntArray(
                MATRIX_ROW_MAX,
                MATRIX_COL_MAX,
                null
            );
            ...
            this.runAction(
                cc.Sequence.create(
                    cc.DelayTime.create(gInitMatrixDelayTime),
    ...
    initMatrix: function() {
        ...
        for (row = 0; row < this.m_nMatrixRow; row++) {
            for (col = 0; col < this.m_nMatrixCol; col++) {
                this.mPatternsPos[row][col] = cc.p(
                    baseX + col * space,
                    baseY + row * space
                );
        ...
        this.runAction(
            cc.Sequence.create(
                cc.DelayTime.create(gPatternsFallTime + 0.1),
    ...
    addOnePattern: function(row, col) {
        ...
        if (this.m_nFreezeProbability != 0 && prob < this.m_nFreezeProbability)
            ...
            prob < this.m_nFreezeProbability + this.m_nStoneProbability
        )
            attr = ePatternExtraAttr.Stone;
        else if (
            this.m_nBombProbability != 0 &&
            prob <
                this.m_nFreezeProbability +
                    this.m_nStoneProbability +
                    this.m_nBombProbability
        )
        ...
        this.mPatternsSpr[row][col].setPosition(
            this.mPatternsPos[row][col].x,
            this.mPatternsPos[row][col].y + 400.0
        );
        this.mPatternsSpr[row][col].moveTo(
            gPatternsFallTime,
            this.mPatternsPos[row][col]
        );
    ...
    initArrayFrames: function() {
        ...
        var frame;
        for (var i = 0; i < 18; i++) {
            frame = gSpriteFrameCache.getSpriteFrame(
                'pattern_destroy_' + ('00' + i).slice(-2) + '.png'
            );
        ...
        for (var i = 0; i < 17; i++) {
            frame = gSpriteFrameCache.getSpriteFrame(
                'pattern_explode_' + ('00' + i).slice(-2) + '.png'
            );
            ...
            if (gGameMode == eGameMode.Timer) {
                for (var row = 0; row < this.m_nMatrixRow; row++) {
                    for (var col = 0; col < this.m_nMatrixCol; col++) {
                        this.mPatternBatchNode.removeChild(
                            this.mPatternsSpr[row][col],
                            true
                        );
                        this.addOnePattern(row, col);
                    }
                ...
                this.runAction(
                    cc.Sequence.create(
                        cc.DelayTime.create(gPatternsFallTime + 0.1),
                        cc.CallFunc.create(
                            this.detectionMatrix.bind(this),
                            this
                        )
                    )
                );
    ...
    onCheckPattern: function(pPattern) {
        if (pPattern != null) {
            ...
            if (this.mFirstCheckPattern === null) {
                ...
                this.mCheckMarkSpr.setPosition(
                    this.mPatternsPos[this.mFirstCheckPattern.m_nRowIndex][
                        this.mFirstCheckPattern.m_nColIndex
                    ]
                );
            } else {
                ...
                if (
                    this.mFirstCheckPattern.m_nRowIndex ==
                    this.mSecondCheckPattern.m_nRowIndex
                ) {
                    if (
                        this.mFirstCheckPattern.m_nColIndex > 0 &&
                        this.mFirstCheckPattern.m_nColIndex - 1 ==
                            this.mSecondCheckPattern.m_nColIndex
                    )
                        isAdjacent = true;
                    else if (
                        this.mFirstCheckPattern.m_nColIndex + 1 <
                    ...
                    if (
                        this.mFirstCheckPattern.m_nRowIndex > 0 &&
                        this.mFirstCheckPattern.m_nRowIndex - 1 ==
                            this.mSecondCheckPattern.m_nRowIndex
                    )
                        isAdjacent = true;
                    else if (
                ...
                if (isAdjacent) {
                    ...
                    this.swapTwoPattern(
                        this.mFirstCheckPattern,
                        this.mSecondCheckPattern,
                        false
                    );
                    ...
                    this.mCheckMarkSpr.setPosition(
                        this.mPatternsPos[this.mSecondCheckPattern.m_nRowIndex][
                            this.mSecondCheckPattern.m_nColIndex
                        ]
                    );
    ...
    onSwapTwoPattern: function(pPattern) {
        if (pPattern) {
            ...
            switch (pFirstCheckPattern.m_eSwapDirection) {
                case eSwapDirection.Left:
                    if (pFirstCheckPattern.m_nColIndex > 0)
                        pSecondCheckPattern = this.mPatternsSpr[
                            pFirstCheckPattern.m_nRowIndex
                        ][pFirstCheckPattern.m_nColIndex - 1];
                    break;
                ...
                case eSwapDirection.Up:
                    if (pFirstCheckPattern.m_nRowIndex + 1 < this.m_nMatrixRow)
                        pSecondCheckPattern = this.mPatternsSpr[
                            pFirstCheckPattern.m_nRowIndex + 1
                        ][pFirstCheckPattern.m_nColIndex];
                    break;
            ...
            if (
                pSecondCheckPattern &&
                pSecondCheckPattern.g_ePatternStatus == ePatternStatus.Normal
            ) {
    ...
    swapTwoPattern: function(firstPattern, secondPattern, isRecover) {
        ...
        secondPattern.swapTo(
            gPatternsSwapTime,
            this.mPatternsPos[fpRow][fpCol]
        );
        ...
        this.runAction(
            cc.Sequence.create(
                cc.DelayTime.create(gPatternsSwapTime),
                cc.CallFunc.create(
                    this.onSwapFinish.bind(this),
                    this,
                    firstPattern
                )
            )
    ...
    onSwapFinish: function(pnode, pPattern) {
        ...
        if (pfPattern.g_bIsRecover) this.onClearFinish(null, null);
        else {
            var matrixMark = this.createIntArray(
                this.m_nMatrixRow,
                this.m_nMatrixCol,
                0
            ); //[[]];
            ...
            if (
                this.getResultByPoint(
                    pfPattern.m_nRowIndex,
                    pfPattern.m_nColIndex,
                    matrixMark
                ) |
                this.getResultByPoint(
                    psPattern.m_nRowIndex,
                    psPattern.m_nColIndex,
                    matrixMark
                )
            )
    ...
    clearSomePatterns: function(matrixMark) {
        ...
        for (var row = 0; row < this.m_nMatrixRow; row++) {
            for (var col = 0; col < this.m_nMatrixCol; col++) {
                if (
                    ...
                    this.mPatternsSpr[row][col].g_ePatternStatus !=
                        ePatternStatus.Normal
                )
                    continue;
                ...
                switch (matrixMark[row][col]) {
                    case 1:
                        ...
                        this.mPatternsSpr[row][
                            col
                        ].g_nRemoveBatchIndex = this.mDestroyBatchTally;
                        tally++;
                        break;
                    case 2:
        ...
        if (tally != 0) {
            ...
            this.updateProgress();
            if (this.mMultipleTimer > 0.0) this.mScoreMultiple++;
            this.mMultipleTimer = 3.0;
        }

        this.runAction(
            cc.Sequence.create(
                cc.DelayTime.create(gPatternsClearTime),
                cc.CallFunc.create(
                    this.onClearFinish.bind(this),
                    this,
                    this.mDestroyBatchTally
                )
            )
    ...
    onClearFinish: function(pnode, removeIndex) {
        ...
        for (col = 0; col < this.m_nMatrixCol && removeBatchIndex; col++) {
            for (row = 0; row < this.m_nMatrixRow; row++) {
                if (
                    this.mPatternsSpr[row][col] &&
                    this.mPatternsSpr[row][col].g_nRemoveBatchIndex ==
                        removeBatchIndex
                ) {
                    this.mPatternBatchNode.removeChild(
                        this.mPatternsSpr[row][col],
        ...
        for (col = 0; col < this.m_nMatrixCol; col++) {
            for (row = 0; row < this.m_nMatrixRow; row++) {
                if (
                    row == 0 &&
                    this.mPatternsSpr[row][col] &&
                    this.mPatternsSpr[row][col].m_eExtraAttr ==
                        ePatternExtraAttr.Stone
                ) {
                    this.mPatternsSpr[row][col].runAction(
                        cc.Sequence.create(
                            cc.MoveBy.create(
                                gPatternsFallTime,
                                cc.p(0.0, -400.0)
                ...
                if (this.mPatternsSpr[row][col] == null) {
                    ...
                    for (var n = row + 1; n < this.m_nMatrixRow; n++) {
                        if (this.mPatternsSpr[n][col] != null) {
                            if (
                                row == 0 &&
                                this.mPatternsSpr[n][col].m_eExtraAttr ==
                                    ePatternExtraAttr.Stone
                            ) {
                                this.mPatternsSpr[n][col].runAction(
                                    cc.Sequence.create(
                                        cc.MoveBy.create(
                                            gPatternsFallTime,
                                            cc.p(0.0, -400.0)
                    ...
                    if (notnull_r != -1) {
                        if (
                            this.mPatternsSpr[notnull_r][col]
                        ...
                        this.mPatternsSpr[notnull_r][col].moveTo(
                            (notnull_r - row) * 0.1,
                            this.mPatternsPos[row][col]
                        );
                        this.mPatternsSpr[row][col] = this.mPatternsSpr[
                            notnull_r
                        ][col];
                        this.mPatternsSpr[row][col].m_nRowIndex = row;
                        this.mPatternsSpr[row][col].m_nColIndex = col;
    ...
    detectionMatrix: function() {
        ...
        if (this.clearSomePatterns(matrixMark) == 0) {
            ...
            if (bRet) this.excludeDeadlock();
        }
    },
    updateTimerForPrompt: function(dt) {
        this.mPromptTimerTally += dt;
        if (this.mMultipleTimer > 0.0) this.mMultipleTimer -= dt;
        else {
        ...
        if (this.mPromptTimerTally >= 10.0) {
            ...
            if (bRet) {
                this.mPromptTimerTally = 0;
                this.isHasSolution();
                this.mPromptMarkSpr.setPosition(
                    this.mPromptPattern.getPosition()
                );
    ...
    updateProgress: function() {
        ...
        switch (gGameMode) {
            case eGameMode.Challenge: {
                var penergyPercent =
                    this.mPatternClearTally / this.mPatternRequire;

                if (penergyPercent > 1.0) penergyPercent = 1.0;
                else if (penergyPercent < 0.0) penergyPercent = 0.0;
                var vh = 326 * penergyPercent;
                this.mVisibleRect = cc.rect(0, 326 - vh, 18, vh);
                this.mProgressSpr.setTextureRect(this.mVisibleRect);

                if (penergyPercent == 1.0) this.showGameResult(true);
                break;
            }
            case eGameMode.Timer: {
                var penergyPercent = 0.0;
                penergyPercent =
                    (this.mTimeTotal - this.mTimerTally) / this.mTimeTotal;
                ...
                if (penergyPercent == 0.0) {
                    ...
                    else this.showGameResult(false);
                }
                break;
            }
        }
    ...
    showGameResult: function(isPass) {
        ...
        resultLayer.initResultData(
            this.mGameScore,
            this.mPatternRequire * 100 * 3,
            gScoreData.bestScore,
            isPass
        );
        this.onExit();
        gScoreData.setLastScore(this.mGameScore);

        if (isPass) gSharedEngine.playEffect(EFFECT_GAME_WIN);
        else gSharedEngine.playEffect(EFFECT_GAME_FAIL);

        cc.Director.getInstance()
            .getRunningScene()
            .addChild(resultLayer, 99);
    },
    getResultByPoint: function(row, col, matrixMark) {
        if (this.mPatternsSpr[row][col] == null) return false;
        ...
        while (i >= 0) {
            if (
                this.mPatternsSpr[row][i] &&
                this.mPatternsSpr[row][i].g_ePatternStatus ==
                    ePatternStatus.Normal &&
                ...
                switch (this.mPatternsSpr[row][i].m_eExtraAttr) {
                    case ePatternExtraAttr.Bomb: {
                        ...
                        if (i + 1 < this.m_nMatrixCol) {
                            matrixMark[row][i + 1] = 3;
                            if (row > 0) matrixMark[row - 1][i + 1] = 3;
                            if (row + 1 < this.m_nMatrixRow)
                                matrixMark[row + 1][i + 1] = 3;
                        }
                        break;
                    ...
                    case ePatternExtraAttr.Freeze:
                        if (matrixMark[row][i] != 3) matrixMark[row][i] = 2;
                        break;
                    default:
                        if (matrixMark[row][i] == 0) matrixMark[row][i] = 1;
                        break;
                }
            }
            bRet = true;
        ...
        while (i >= 0) {
            if (
                this.mPatternsSpr[i][col] &&
                this.mPatternsSpr[i][col].g_ePatternStatus ==
                    ePatternStatus.Normal &&
        ...
        if (count >= MATRIX_CONTINUOUS) {
            for (i = start; i <= end; i++) {
                switch (this.mPatternsSpr[i][col].m_eExtraAttr) {
                    ...
                    case ePatternExtraAttr.Freeze:
                        if (matrixMark[i][col] != 3) matrixMark[i][col] = 2;
                        break;
                    default:
                        if (matrixMark[i][col] == 0) matrixMark[i][col] = 1;
                        break;
                }
            }
            bRet = true;
        ...
        for (var row = 0; row < this.m_nMatrixRow; row++) {
            for (var col = 0; col < this.m_nMatrixCol - 1; col++) {
                if (
                    ...
                    if (
                        targetType ==
                        this.mPatternsSpr[row][col + 1].m_ePatternType
                    ) {
                        //  *
                        //**
                        //  *
                        if (
                            row > 0 &&
                            col + 2 < this.m_nMatrixCol &&
                            this.mPatternsSpr[row - 1][col + 2]
                                .m_ePatternType == targetType
                        ) {
                            if (this.mPatternsSpr[row][col + 2].m_bSwapEnable) {
                                this.mPromptPattern = this.mPatternsSpr[row][
                                    col + 2
                                ];
                                return true;
                            }
                            if (
                                ...
                                this.mPromptPattern = this.mPatternsSpr[
                                    row - 1
                                ][col + 2];
                                return true;
                            }
                        ...
                        if (
                            col - 2 >= 0 &&
                            this.mPatternsSpr[row][col - 2].m_ePatternType ==
                                targetType
                        ) {
                            if (this.mPatternsSpr[row][col - 2].m_bSwapEnable) {
                                this.mPromptPattern = this.mPatternsSpr[row][
                        ...
                        if (
                            col + 3 < this.m_nMatrixCol &&
                            this.mPatternsSpr[row][col + 3].m_ePatternType ==
                                targetType
                        ) {
        ...
        for (var col = 0; col < this.m_nMatrixCol; col++) {
            for (var row = 0; row < this.m_nMatrixRow - 1; row++) {
                ...
                if (
                    this.mPatternsSpr[row][col].m_eExtraAttr !=
                        ePatternExtraAttr.Stone &&
                    targetType == this.mPatternsSpr[row + 1][col].m_ePatternType
                ) {
                    // ? ?
                    //  x
                    //  x
                    if (
                        col > 0 &&
                        row + 2 < this.m_nMatrixRow &&
                        this.mPatternsSpr[row + 2][col - 1].m_ePatternType ==
                            targetType
                    ) {
                        if (this.mPatternsSpr[row + 2][col - 1].m_bSwapEnable) {
                            this.mPromptPattern = this.mPatternsSpr[row + 2][
                                col - 1
                            ];
                            return true;
                        }
                    ...
                    if (
                        col > 0 &&
                        row > 0 &&
                        this.mPatternsSpr[row - 1][col - 1].m_ePatternType ==
                            targetType
                    ) {
                        if (this.mPatternsSpr[row - 1][col - 1].m_bSwapEnable) {
                            this.mPromptPattern = this.mPatternsSpr[row - 1][
                                col - 1
                            ];
                            return true;
                        }
                    ...
                    if (
                        row - 2 >= 0 &&
                        this.mPatternsSpr[row - 2][col].m_ePatternType ==
                            targetType
                    ) {
                        if (this.mPatternsSpr[row - 2][col].m_bSwapEnable) {
                            this.mPromptPattern = this.mPatternsSpr[row - 2][
                    ...
                    if (
                        row + 3 < this.m_nMatrixRow &&
                        this.mPatternsSpr[row + 3][col].m_ePatternType ==
                            targetType
                    ) {
                        if (this.mPatternsSpr[row + 3][col].m_bSwapEnable) {
                            this.mPromptPattern = this.mPatternsSpr[row + 3][
                                col
                            ];
                            return true;
                        }
