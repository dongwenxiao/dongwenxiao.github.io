...
var MyShareResult = plugin.ShareResultListener.extend({
    onShareResult: function(ret, msg) {
        var shareStatus =
            'Share ' + (ret == plugin.ShareResultCode.Success)
                ? ' Successed'
                : ' Failed';
...
var MySocialManager = cc.Class.extend({
    ...
    loadSocialPlugin: function() {
        ...
        this._snweibo = plugin.PluginManager.getInstance().loadPlugin(
            'SocialWeibo'
        );
        if (this._snweibo) {
            var weiboInfo = {};
            ...
            if (Object.keys(weiboInfo).length == 0) {
                cc.log(
                    'Developer info is empty. PLZ fill your weibo info in weiboInfo'
                );
            }
        ...
        this._qqweibo = plugin.PluginManager.getInstance().loadPlugin(
            'SocialQQWeibo'
        );
        if (this._qqweibo) {
            var qqweiboInfo = {};
            ...
            if (Object.keys(qqweiboInfo).length == 0) {
                cc.log(
                    'Developer info is empty. PLZ fill your weibo info in qqweiboInfo'
                );
            }
        ...
        this._qzone = plugin.PluginManager.getInstance().loadPlugin(
            'SocialQzone'
        );
        this._twitter = plugin.PluginManager.getInstance().loadPlugin(
            'SocialTwitter'
        );
        this._facebook = plugin.PluginManager.getInstance().loadPlugin(
            'SocialFacebook'
        );
