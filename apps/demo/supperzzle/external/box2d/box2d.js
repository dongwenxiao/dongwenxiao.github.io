...
(function() {
    ...
    b2Collision.FindMaxSeparation = function(
        ...
        var sPrev = b2Collision.EdgeSeparation(
            poly1,
            xf1,
            prevEdge,
            poly2,
        ...
        var sNext = b2Collision.EdgeSeparation(
            poly1,
            xf1,
            nextEdge,
            poly2,
    ...
    b2Collision.CollidePolygons = function(manifold, polyA, xfA, polyB, xfB) {
        ...
        for (var i = 0; i < b2Settings.b2_maxManifoldPoints; ++i) {
            cv = clipPoints2[i];
            var separation =
                normal.x * cv.v.x + normal.y * cv.v.y - frontOffset;
    ...
    b2Distance.Distance = function(output, cache, input) {
        ...
        while (iter < k_maxIters) {
            ...
            vertex.wA = b2Math.MulX(
                transformA,
                proxyA.GetVertex(vertex.indexA)
            );
            vertex.indexB = proxyB.GetSupport(b2Math.MulTMV(transformB.R, d));
            vertex.wB = b2Math.MulX(
                transformB,
        ...
        if (input.useRadii) {
            ...
            if (
                ...
                output.distance > Number.MIN_VALUE
            ) {
                output.distance -= rA + rB;
    ...
    b2DistanceProxy.prototype.Set = function(shape) {
        switch (shape.GetType()) {
            ...
            case b2Shape.e_polygonShape:
                {
                    var polygon =
                        shape instanceof b2PolygonShape ? shape : null;
    ...
    b2DynamicTree.prototype.InsertLeaf = function(leaf) {
        ...
        if (sibling.IsLeaf() == false) {
            do {
                ...
                var norm1 =
                    Math.abs(
                        (child1.aabb.lowerBound.x + child1.aabb.upperBound.x) /
                            2 -
                            center.x
                    ) +
                    Math.abs(
                        (child1.aabb.lowerBound.y + child1.aabb.upperBound.y) /
                            2 -
                            center.y
                    );
                var norm2 =
                    Math.abs(
                        (child2.aabb.lowerBound.x + child2.aabb.upperBound.x) /
                            2 -
                            center.x
                    ) +
                    Math.abs(
                        (child2.aabb.lowerBound.y + child2.aabb.upperBound.y) /
                            2 -
                            center.y
                    );
                if (norm1 < norm2) {
        ...
        if (count == 1) {
            ...
            if (s == 0.0 || s == 1.0) {
                ...
                pointAY =
                    ...
                    (tMat.col1.y * tVec.x + tMat.col2.y * tVec.y);
                sgn =
                    (pointAX - pointBX) * normalX +
                    (pointAY - pointBY) * normalY;
                if (s < 0.0) {
                    this.m_axis.NegativeSelf();
                }
            } else {
                ...
                pointBY =
                    ...
                    (tMat.col1.y * tVec.x + tMat.col2.y * tVec.y);
                sgn =
                    (pointBX - pointAX) * normalX +
                    (pointBY - pointAY) * normalY;
                if (s < 0.0) {
                    this.m_axis.NegativeSelf();
                }
            }
...
(function() {
    ...
    b2Body.prototype.ResetMassData = function() {
        ...
        if (
            ...
            this.m_I -=
                this.m_mass * (center.x * center.x + center.y * center.y);
    ...
    b2Island.prototype.Solve = function(step, gravity, allowSleep) {
        ...
        for (i = 0; i < this.m_bodyCount; ++i) {
            ...
            if (
                ...
                b.m_linearVelocity.x *=
                    b2Settings.b2_maxTranslation * step.inv_dt;
                b.m_linearVelocity.y *=
                    b2Settings.b2_maxTranslation * step.inv_dt;
            }
        ...
        if (allowSleep) {
            ...
            for (i = 0; i < this.m_bodyCount; ++i) {
                ...
                if (
                    ...
                    b2Math.Dot(b.m_linearVelocity, b.m_linearVelocity) >
                        linTolSqr
                ) {
                    b.m_sleepTime = 0.0;
                    minSleepTime = 0.0;
    ...
    b2Island.prototype.SolveTOI = function(subStep) {
        ...
        for (i = 0; i < this.m_bodyCount; ++i) {
            ...
            if (rotation * rotation > b2Settings.b2_maxRotationSquared) {
                if (b.m_angularVelocity < 0.0) {
                    b.m_angularVelocity =
                        -b2Settings.b2_maxRotation * subStep.inv_dt;
                } else {
                    b.m_angularVelocity =
                        b2Settings.b2_maxRotation * subStep.inv_dt;
                }
    ...
    b2Island.prototype.Report = function(constraints) {
        ...
        for (var i = 0; i < this.m_contactCount; ++i) {
            ...
            for (var j = 0; j < cc.pointCount; ++j) {
                b2Island.s_impulse.normalImpulses[j] =
                    cc.points[j].normalImpulse;
                b2Island.s_impulse.tangentImpulses[j] =
                    cc.points[j].tangentImpulse;
            }
                ...
                if (c.m_flags & b2Contact.e_toiFlag) {
                    ...
                    if (
                        (bA.GetType() != b2Body.b2_dynamicBody ||
                            bA.IsAwake() == false) &&
                        (bB.GetType() != b2Body.b2_dynamicBody ||
                            bB.IsAwake() == false)
                    ) {
                        continue;
                    }
            ...
            case b2Shape.e_polygonShape:
                {
                    ...
                    this.m_debugDraw.DrawSolidPolygon(
                        vertices,
                        vertexCount,
                        color
                    );
    ...
    b2ContactSolver.prototype.Initialize = function(
        ...
        for (i = 0; i < contactCount; ++i) {
            ...
            for (var k = 0; k < cc.pointCount; ++k) {
                ...
                var kEqualized =
                    bodyA.m_mass * bodyA.m_invMass +
                    bodyB.m_mass * bodyB.m_invMass;
                kEqualized +=
        ...
        for (var i = 0; i < this.m_constraintCount; ++i) {
            ...
            if (step.warmStarting) {
                ...
                for (j = 0; j < tCount; ++j) {
                    ...
                    var PX =
                        ccp.normalImpulse * normalX +
                        ccp.tangentImpulse * tangentX;
                    var PY =
                        ccp.normalImpulse * normalY +
                        ccp.tangentImpulse * tangentY;
    ...
    b2TensorDampingController.prototype.Step = function(step) {
        ...
        for (var i = this.m_bodyList; i; i = i.nextBody) {
            ...
            var damping = body.GetWorldVector(
                b2Math.MulMV(
                    this.T,
                    body.GetLocalVector(body.GetLinearVelocity())
                )
            );
            body.SetLinearVelocity(
                new b2Vec2(
            ...
            case b2Joint.e_gearJoint:
                {
                    joint = new b2GearJoint(
                        def instanceof b2GearJointDef ? def : null
                    );
                }
                break;
            case b2Joint.e_lineJoint:
                {
                    joint = new b2LineJoint(
                        def instanceof b2LineJointDef ? def : null
                    );
                }
                break;
            case b2Joint.e_weldJoint:
                {
                    joint = new b2WeldJoint(
                        def instanceof b2WeldJointDef ? def : null
                    );
                }
                break;
    ...
    b2LineJoint.prototype.InitVelocityConstraints = function(step) {
        ...
        }
        {
            ...
            this.m_K.col1.x =
                m1 +
                m2 +
                i1 * this.m_s1 * this.m_s1 +
                i2 * this.m_s2 * this.m_s2;
            this.m_K.col1.y =
            ...
            this.m_K.col2.y =
                m1 +
                m2 +
                i1 * this.m_a1 * this.m_a1 +
                i2 * this.m_a2 * this.m_a2;
        }
